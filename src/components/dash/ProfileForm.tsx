    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <button type="button" onClick={() => picker.current?.click()} className="text-[11.5px] font-semibold text-brand-400 hover:underline">
                Change photo
              </button>
              {preview && !removed && (
                <button type="button" onClick={() => { setRemoved(true); setPreview(null); }} className="flex items-center gap-1 text-[11.5px] muted transition-colors hover:text-rose-400">
      ...
      <Section title="Personal information">
        <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2">
          <input ref={picker} type="file" name="avatarFile" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) void pickAvatar(f); }} />
