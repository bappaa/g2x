...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const gamesList = await all(`SELECT slug, name FROM games WHERE status='active' ORDER BY name`);
  const catsList = await all(`SELECT slug, name FROM categories WHERE status='active' ORDER BY name`);
  ...
  return (
    <div className="space-y-4 max-w-[720px] mx-auto">
      ...
      <EditListingClient listing={listing} games={gamesList as any} categories={catsList as any} />
    </div>
  );
}
